...
socket.once('firstupdate', function(fullListOfUsers, enviroStates)
{
    ...
    {
        var eCurrent = enviroStates[eKey];
        
        if (!eCurrent) {
            var sceneObj = document.getElementById(eKey);
            var mat = sceneObj.getElementsByTagName("Material");
 ...
 */
socket.on('updateEnvironment', function(eObject) {
    
    //eObject.updateClient();
    
    var lightBulb = document.getElementById(eObject);
